...
using WriteInput = std::expected<std::string, std::error_condition>;

using WriterFn = std::move_only_function<void(Proc&, WriteInput)>;
using ReaderFn = std::move_only_function<Task<ReadResult>(Proc&)>;
