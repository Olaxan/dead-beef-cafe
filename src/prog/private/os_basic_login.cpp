...
ProcessTask Programs::CmdLogin(Proc& proc, std::vector<std::string> args)
{
	...
	proc.put(" | Username: ");
	std::string username = co_await proc.read(); //CmdInput::read_cmd_utf8(proc, usr_params);

	proc.put(" | Password: ");
	std::string password = co_await proc.read(); //CmdInput::read_cmd_utf8(proc, pwd_params);
